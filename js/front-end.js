...
function pprf_new_fn( podid, postid, cpodid, authorid , iframeid, poditemid, parentName ){
	if( jQuery.isNumeric( podid ) && jQuery.isNumeric( cpodid ) && jQuery.isNumeric( authorid ) && jQuery.isNumeric( poditemid ) ) {
		...
		var para_obj  = { 'podid': podid, 'postid': postid, 'cpodid': cpodid, 'authorid': authorid, 'poditemid' : poditemid, 'action' : 'front_pprf_load_newly_added_fn', 'security' : ajax_script.nonce };
		...
		jQuery.post(
			...
			function( return_str ){	
				...
				if( typeof return_arr['id'] != 'undefined' && jQuery.isNumeric( return_arr['id'] ) ){
					...
					var iframe_str   	= '<li data-id="' + response_str + '" class="pprf-not-trashed" id="li-' + ids_str + '" style="display:block">' +
...
function pprf_delete_item_fn( podid, postid, cpodid, itemid, authorid , iframeid, poditemid, trashed ){
	...
	if( jQuery.isNumeric( podid ) && jQuery.isNumeric( cpodid ) && jQuery.isNumeric( authorid ) && jQuery.isNumeric( itemid ) && jQuery.isNumeric( poditemid )  ) {
		...
		var para_obj  	= { 'podid': podid, 'postid': postid, 'cpodid': cpodid, 'itemid' : itemid, 'authorid': authorid, 'poditemid' : poditemid, 'action' : 'front_pprf_delete_item_fn', 'trash' : trashed, 'security' : ajax_script.nonce };
		var info_str	=	'';
		if( trashed == 0 ){
...
jQuery(document).ready( function($) {
	...
	updateIndex_fn = function(e, ui) {
		...
		var data = {
			action:   'front_pprf_update_order_fn',
			order: 	  theOrder_arr, 
