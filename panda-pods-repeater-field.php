 ...
 *
 * Panda_Pods_Repeater_Field The class that holds the entire Panda_Pods_Repeater_Field plugin
 *
 * @since 1.0.0
 */
 ...
 */
function pandarf_pods_data( $tb_str, $searches = array(
	'pod_id'       => '',
	'post_id'      => '',
	'pod_field_id' => '',
), $params_arr = array() ) {
...
function is_pandarf( $field_name, $parent_id = 0 ) {
	...
	if ( false === $pandarf_field ) {
		...
		$query = $wpdb->prepare(
			'SELECT ps_tb.ID, ps_tb.post_name, ps_tb.post_title, ps_tb.post_author, ps_tb.post_parent 
										 ...
										 WHERE ps_tb.`post_type` = "_pods_field" AND ps_tb.`post_name` = "%s" ' . $where . ' LIMIT 0, 1',
			$params
		);
...
function pprf_enqueue_scripts() {
	...
	if ( version_compare( $wp_version, '5.9', '=' ) ) {
		...
		wp_register_script( 'panda-pods-repeater-scripts', plugins_url( 'js/admin.min.js', __FILE__ ), array( 'jquery', 'jquery-ui-core', 'jquery-ui-resizable', 'jquery-ui-draggable', 'jquery-ui-droppable', 'jquery-ui-sortable', 'panda-pods-repeater-jquery-ui' ), false, true );
	} else {
		wp_register_script( 'panda-pods-repeater-scripts', plugins_url( 'js/admin.min.js', __FILE__ ), array( 'jquery', 'jquery-ui-core', 'jquery-ui-resizable', 'jquery-ui-draggable', 'jquery-ui-droppable', 'jquery-ui-sortable' ), false, true );
	}
